...
wigout = open(os.path.relpath(clargs.outputdir) + "/WellingtonFootprints.wig","w")
fdrout = open(os.path.relpath(clargs.outputdir) + "/WellingtonFootprints.FDR.bed","w")
...
def writetodisk(fp):
    #Raw WIG scores
    holdstring="fixedStep\tchrom=" + str(fp.interval.chromosome) + "\t start="+ str(fp.interval.startbp) +"\tstep=1\n"
    wigout.write(holdstring)
