...
#Simplified file names for ease of automated access
wigout = open(os.path.relpath(args.outputdir) + "/WellingtonFootprints.wig","w")
fdrout = open(os.path.relpath(args.outputdir) + "/WellingtonFootprints.FDR.bed","w")

#Required for UCSC upload
wigout.write("track type=wiggle_0\n")
...
for each in progress.bar(orderedbychr):
    ...
    holderstring = "fixedStep\tchrom=" + str(fp.interval.chromosome) + "\t start="+ str(fp.interval.startbp) +"\tstep=1\n"
    wigout.write(holderstring)
    for i in fp.scores:
        wigout.write(str(i))
        wigout.write('\n')
